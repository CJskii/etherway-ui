export const handleErrors = ({
  ...
  if (dataMessage) {
    ...
    } else if (genericMessage.includes("hex data is odd-length")) {
      return setErrorMessage(
        "Invalid recipent address provided. Please check and try again.",
      );
    } else if (genericMessage.includes("quoteSend")) {
      return setErrorMessage(
        "Failed to get quote. Please lower input amount and try again.",
      );
    } else {
      return setErrorMessage(genericMessage);
    }
  } else {
